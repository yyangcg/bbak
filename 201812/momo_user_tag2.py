...
# def momo_user_tag(data_path,arguments):
#     sqlContext = HiveContext(sc)
#     sqlContext.sql("use usercenter_dw")
...
#         .withColumn('update_dt', lit(cur_date))
#     if month != None:
#         df = df.withColumn('etl_month', lit(arguments['month']))
#     if date != None:
#         df = df.withColumn('etl_dt', lit(arguments['date']))
#     df.registerTempTable('tab_name')
#     sqlContext.sql("insert into table momo_user_tag select * from tab_name")
#
# def parse_arguments(arguments):
#     parser = argparse.ArgumentParser(description='''timestamp''', formatter_class=RawTextHelpFormatter)
#     parser.add_argument("--month", required=True, help='用户数据月份')
#     parser.add_argument("--date", required=True, help='用户数据日期')
#     try:
...
def get_data(from_i,to_i):
    ...
    for i in range(from_i,to_i):
       date = time.strftime("%Y-%m-%d", time.localtime(int(time.time() - i * 60 * 60 * 24)))
       sqlContext.sql("insert into table usercenter_dw.yyh_momo_tmp select distinct imeimd5 as userid, mediadata,etl_dt from t2pdm_data.t05_chehui_dsp_log_v2 "
                      "where etl_dt ='"+ date + "' and channelid=4 and length(imeimd5) = 32")
       print("finished",date)
...
if __name__ == '__main__':
    ...
    #for i in range(27,37):
     #   date = time.strftime("%Y-%m-%d", time.localtime(int(time.time() - i * 60 * 60 * 24)))
      #  sqlContext.sql("insert into table usercenter_dw.yyh_momo_tmp select distinct imeimd5 as userid, mediadata,etl_dt from t2pdm_data.t05_chehui_dsp_log_v2 where etl_dt ='"+ date + "' and channelid=4 and length(imeimd5) > 10")
       # print("finished",i)
